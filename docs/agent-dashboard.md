...
- The original census returns as multi-year line graphs for Wikimedia readership, GitHub PRs and crawler policies. Only the selected graph is mounted. Homepage bar graphs are removed.
- A daily GitHub heatmap adds year selection, a consistent color scale across years, mouse/keyboard inspection and distinct missing, zero and incomplete observations.
...
The census displays full stored monthly and policy history, including the Wikimedia baseline back to 2015 where available. GitHub shares require comparable validated archive periods. Missing Wikimedia months remain gaps, including trailing missing months. Radar remains on the Traffic page with the publisher's units, normalization, window and retrieval time. Policy charts use v2 explicit named-token full-block definitions without joining legacy definitions into their line; directives do not establish crawler compliance.

Independent reads run together and shared report/flow data is cached for five minutes. The homepage mounts one census graph and one selected calendar year, and uses paths instead of a DOM node per historical point. It does not render hidden full-history charts. Heatmap ratios require comparable daily source coverage; current UTC days are excluded.
...
The observed homepage uses configured external-source data; it never substitutes examples when unavailable. `/demo` uses fixed synthetic data for its flow, historical series and heatmap and is labelled and marked noindex.
...
Unit checks cover missing-month handling, withheld legacy archive shares, line gaps, distinct heatmap states, unique hatch patterns, disabled local analytics and independent flow scales. PostgreSQL checks verify that private local visit fixtures cannot enter the flow or latest external records, alongside UTC boundaries, collection failure and the coding-agent remainder.

Browser checks cover the restored rail, census tabs, heatmap year/date selection, retired URLs, absence of local dashboard metrics, navigation, exports, saved records, hydration, animation and reduced motion, light/dark layouts, and widths of 390, 768, 1024 and 1440 pixels.
...
![Restored multi-year census, daily heatmap and left navigation at 1440 pixels](qa/dashboard-demo-1440.png)
