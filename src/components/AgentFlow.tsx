    ...
    <div className="flow" data-mode={data.mode}>
      ...
      <div className="chart-scroll flow-scroll" tabIndex={0} role="region" aria-label="Agents to destinations; scroll horizontally on small screens">
        <svg className="chart flow-svg" viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Agents and observed activity connected to their destinations">
          <text x={left - 12} y={20} textAnchor="end" className="flow-heading">AGENTS &amp; OBSERVED ACTIVITY</text>
          <text x={right + 14} y={20} className="flow-heading">DESTINATIONS</text>
          ...
          {links.map((link, i) => {
            ...
            const dots = Math.max(1, Math.ceil(weight * 5));
          ...
          {!links.length ? <text x={W/2} y={H/2} textAnchor="middle" className="flow-label">No observations available in this window.</text> : null}
        </svg>
      </div>
      <p className="flow-scale-note">Width and dot density compare counts only within the same source and unit. Each source uses its own scale. PR matches may overlap; edits and files stay separate. Animation illustrates recorded activity, not live event timing. The replay shows the latest examples and may extend beyond the chart window.</p>
      {selected ? <section className="flow-inspector" aria-label="Source evidence and coverage">
        ...
        <dl className="flow-metadata">
          ...
          <div><dt>Observed dates</dt><dd>{selected.observedDays}/{data.days} UTC dates contain rows</dd></div>
